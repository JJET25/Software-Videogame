...
export default class MiniMap {
    ...
    draw(renderer) {
        ...
        // Draw connections 
        const drawEdges = new Set(); // Prevents double conections as A->B and B->A
        for (const node of nodes){
            const posA = this.#toScreenPos(node.gridPos, minX, minY, originX, originY);
            ...
            for (const neighbor of graph.getNeighbors(node.id)){
                ...
                const posB = this.#toScreenPos(neighbor.gridPos, minX, minY, originX, originY);
        ...
        // Draw nodes
        for (const node of nodes){
            const screenPos = this.#toScreenPos(node.gridPos, minX, minY, originX, originY);
    ...
    #drawConnection(renderer, posA, posB) {
        ...
        renderer.drawLine(
            posA.x + half, posA.y + half, 
            posB.x + half, posB.y + half, 
            this.config.colors.connection, this.config.lineWidth);
    }
}