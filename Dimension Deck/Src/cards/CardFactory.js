...
const ACTIVE_MAP = {
    ...
    defense: (d, r) => new ActiveDefenseCard({
        name:         d.card_name,
        description:  d.description,
        rarity:       r,
...
export function createCard(dbData) {
    ...
    if (dbData.card_type === 'automatic') {
        ...
        if (!trigger) {
            console.warn(`CardFactory: unknown trigger "${dbData.effect_json?.trigger}" for "${dbData.card_name}"`);
            return null;
        }
    ...
    if (!factory) {
        console.warn(`CardFactory: unknown subtype "${dbData.subtype}" for "${dbData.card_name}"`);
        return null;
    }
