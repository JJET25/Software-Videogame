...
export default class MiniMap {
    ...
    draw(renderer) {
        ...
        const RoomManager =
            this.dimensionManager.roomManager;
        ...
        const minX = Math.min(
            ...nodes.map(node => node.gridPos.x)
        );

        const minY = Math.min(
            ...nodes.map(node => node.gridPos.y)
        );
