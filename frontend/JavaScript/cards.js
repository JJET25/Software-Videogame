...
const CARDS = [
  ...
  },
  {
    ...
    rarity: "epic",
    description:
      "Full-circle explosion dealing 120 damage to all enemies within 72 px.",
    stats: ["120 DMG", "180°", "7 s CD"],
    starter: false,
    image: ACT + "nova-burst.png",
  },
  {
    name: "Shadow Blade",
    type: "active",
    rarity: "legendary",
    description:
      "A devastating strike dealing 250 damage to enemies within 48 px.",
...
function buildCard(card) {
  ...
  const ribbon = card.starter
    ? `<span class="card-starter">STARTER</span>`
    : "";

  const pills = card.stats
...
function applyFilters() {
  document.querySelectorAll(".card-item").forEach((item) => {
    ...
    const matchRarity =
      activeRarity === "all" || item.dataset.rarity === activeRarity;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (user) {
    ...
    document.getElementById("usernameInitial").textContent =
